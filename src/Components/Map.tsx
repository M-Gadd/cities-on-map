...
const Map: React.SFC<MapProps> = ({ location }) => {
  const { showingCities, isLoading, lat, lng } = useApiData(location);
  ...
  const fetchMap = () => {
    if (!isLoading && showingCities) {
      map.setCenter({ lat, lng });
      map.setZoom(11);

      showingCities.forEach((one: any, i: number) => {
  ...
  return (
    <div className="mt-3">
      <Row>
        {showingCities &&
          showingCities.map((city: any, i: number) => (
            <Col>
